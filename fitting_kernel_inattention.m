function Subject  = fitting_kernel_inattention
...
%         try
        ...
        ln_gamma=params_ml(4);
        
        %aic
        aic=2*opt_mllh+2*4;%kappa,xi,gamma,sigma2
        ...
        % results
        Subject(cs).Path(cpath).SKernel.kappa=exp(ln_kappa);
        Subject(cs).Path(cpath).SKernel.xi=exp(ln_xi);
        Subject(cs).Path(cpath).SKernel.gamma=exp(ln_gamma);
        Subject(cs).Path(cpath).SKernel.sigma2=exp(ln_sigma2);
        Subject(cs).Path(cpath).SKernel.llh=-opt_mllh;
        Subject(cs).Path(cpath).SKernel.aic=aic;
        Subject(cs).Path(cpath).SKernel.diff_f=diff_f;
        Subject(cs).Path(cpath).SKernel.n_diff_f=n_diff_f;
        Subject(cs).Path(cpath).SKernel.exitsig=exitsig;
